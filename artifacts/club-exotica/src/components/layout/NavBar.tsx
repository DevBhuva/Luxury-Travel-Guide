    ...
    <motion.nav
      ...
      className={`fixed top-6 left-1/2 -translate-x-1/2 z-50 w-[calc(100%-2rem)] max-w-5xl transition-all duration-500 ease-out px-6 md:px-8 py-3.5 flex items-center justify-between rounded-full border ${
        isScrolled
          ...
          : 'bg-[rgba(18,18,20,0.55)] backdrop-blur-xl border-white/10 shadow-[0_10px_40px_rgba(0,0,0,0.25)]'
      }`}
    >
      <a href="#" className="gold-gradient-text font-serif text-lg md:text-xl tracking-wide flex-shrink-0">
        CLUB EXOTICA
      </a>

      <div className="hidden md:flex gap-10 text-xs font-semibold uppercase tracking-[0.2em]">
        <a href="#destinations" className="text-white/80 hover:text-[var(--gold-light)] transition-colors duration-300">Destinations</a>
        <a href="#experiences" className="text-white/80 hover:text-[var(--gold-light)] transition-colors duration-300">Experiences</a>
        <a href="#membership" className="text-white/80 hover:text-[var(--gold-light)] transition-colors duration-300">Membership</a>
      </div>
